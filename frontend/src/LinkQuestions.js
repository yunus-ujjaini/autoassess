...
function LinkQuestions() {
  ...
  let [linkedQuestions,setLinkedQuestions]=useState([]);
  let [InterviewDuration,setInterviewDuration]=useState(false);
  let [ObjCodingRatio,setObjCodingRatio]=useState(false);
  ...
  useEffect(()=>{
    fetch(`http://localhost:3001/linkQuestions/${selectedJob.value}`)
        .then((response) => response.json())
        .then((data) => {
          console.log(data);
          setLinkedQuestions(data);
          let checkboxes=document.getElementsByClassName("SelectQuestion");
          for(let i=0;i<checkboxes.length;i++){
            checkboxes[i].checked=false;
          }
          data.forEach((question)=>{
            console.log(question);
            if(document.getElementById(question.QuestionTypeId)!=null)
            {
              document.getElementById(question.QuestionTypeId).checked=true;
            }
          })
          let temp = advancedSearch.sort((a,b)=>{
            let indexa=_.findIndex(data,{"QuestionTypeId":a.QuestionTypeId})
            let indexb = _.findIndex(data,{"QuestionTypeId":b.QuestionTypeId})
            console.log(indexa,indexb);
            if(indexa<0 && indexb>=0) return 1
            else if(indexa>=0 && indexb<0) return -1
            else return 0;
          })
          setAdvancedSearch(temp);

          fetch(`http://localhost:3001/jobs/${selectedJob.value}`)
          .then((response) => response.json())
          .then((data) => {
            console.log("Hi,",data);
            setInterviewDuration(data[0].InterviewDuration)
            setObjCodingRatio(data[0].ObjectiveCodingRatio)
          });
  ...
  function navigateToLinkQuestions() {
    navigate("/Interviewer/Dashboard/LinkQuestions");
  }
  ...
  function search(){
    ...
    setQuestions(result);
    let temp = result.sort((a,b)=>{
      let indexa=_.findIndex(linkedQuestions,{"QuestionTypeId":a.QuestionTypeId})
      let indexb = _.findIndex(linkedQuestions,{"QuestionTypeId":b.QuestionTypeId})
      console.log(indexa,indexb);
      if(indexa<0 && indexb>=0) return 1
      else if(indexa>=0 && indexb<0) return -1
      else return 0;
  ...
  function selectQuestion(event,questionId,questionTypeId){
   
    let Allgood=true;
    if(event.target.checked){
      if (selectedJob.length <= 0) {
        alert("Please select Job to start linking questions");
        Allgood=false
        event.preventDefault();
      }
  
      if(Allgood){
        fetch("http://localhost:3001/linkQuestions", {
          method: "POST",
          body: JSON.stringify({
            "QuestionId":questionId,
            "QuestionTypeId": questionTypeId,
            "JobId":selectedJob.value
            // "duration": (questionTypeId.startsWith("F")||questionTypeId.startsWith("M"))?"60s":"600s"
          }),
          headers: {
            "Content-type": "application/json; charset=UTF-8"
          }
  ...
  function changeInterviewDuration(){
    let newDuration=window.prompt("Please enter new duration for Interview or click Cancel",InterviewDuration);
    if(newDuration){
      if(!checkNumber(newDuration)){
        alert("Incorrect Duration Value entered");
      }
      else{
        fetch(`http://localhost:3001/jobs/changeInterviewDuration`, {
        method: "PATCH",
        body: JSON.stringify({
          "InterviewDuration": newDuration,
          "JobId":selectedJob.value
        }),
        headers: {
          "Content-type": "application/json; charset=UTF-8"
  ...
  function changeRatio(){
    let newRatio=window.prompt("Please enter new Objective-Coding ratio for Interview or click Cancel",ObjCodingRatio);
    if(newRatio){
      if(newRatio.split(":").length!==2){
        alert("Incorrect Format entered");
      }
      else if(!(checkNumber(newRatio.split(":")[0]) && checkNumber(newRatio.split(":")[1]))){
        alert("Incorrect Values entered");
      }
      else if((parseInt(newRatio.split(":")[0])+parseInt(newRatio.split(":")[1]))!==100){
        alert("Total should be equal to 100");
      }
      else{
        fetch(`http://localhost:3001/jobs/changeObjCodingRatio`, {
          method: "PATCH",
          body: JSON.stringify({
            "ObjectiveCodingRatio": newRatio,
            "JobId":selectedJob.value
          }),
          headers: {
            "Content-type": "application/json; charset=UTF-8"
    ...
    <>
      ...
      <div className='DashboardContainer'>
        ...
        <div className='DashboardItems'>
          <div className='LinkQuestions-DashboardItem'>
              ...
              menuPortalTarget={document.body}
                styles={{
                  ...
                  theme={(theme) => ({
                    ...
                    <br/>
                    {InterviewDuration && <label onClick={changeInterviewDuration}>Interview Duration: <span>{InterviewDuration}</span> minutes</label>}
                    {InterviewDuration && <label onClick={changeRatio}>Objective-Coding Questions Ratio: <span>{ObjCodingRatio}</span></label>}
            </div>
          </div>
              ...
              {advancedSearch.map(question => {
                return question.QuestionTypeId.startsWith("F")?
                    <div key={question.QuestionTypeId} className='FIBcontentData'  onDoubleClick={()=>{deleteQuestion(question.QuestionId,"Fill in the Blanks",question.QuestionTypeId)}}>
                      <input id={question.QuestionTypeId} className='SelectQuestion' type="checkbox" onChange={(e)=>{selectQuestion(e,question.QuestionId,question.QuestionTypeId)}}/>
